    ...
    public void run() {
       ...
       // session.setDebug(true);// 设置打开调试状态
        ...
        try {
            ...
            // 3邮件内容:主题、内容
            msg.setSubject(user.getUserName() + ",欢迎注册享乐网账号,请点击链接激活账号");

            // StringBuilder是线程不安全的,但是速度快，这里因为只会有这个线程来访问，所以可以用这个
            StringBuilder sbd = new StringBuilder();
            sbd.append(user.getUserName() + "<br/>欢迎！请确认此邮件地址以激活您的账号。<br/>");
            sbd.append("<font color='red'><a href='http://192.168.39.119:8080/pleasure/validate.do?userID="+user.getUserID()+"&ActivateCode="
                    + user.getActivateCode() + "' target='_blank'");
            sbd.append(">立即激活</a></font><br/>");
            sbd.append("或者点击下面链接:<br/>");
            sbd.append("http://192.168.39.119:8080/pleasure/validate.do?userID="+user.getUserID()+"&ActivateCode="
                    + user.getActivateCode() + "<br/>");
